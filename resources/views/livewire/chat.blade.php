            ...
            <div class="lg:col-span-3 flex flex-col gap-2 overflow-auto h-[80vh] sm:h-auto px-2">
                ...
                <div class="sticky top-2 mt-6 z-50">
                    <div class="flex flex-col sm:flex-row items-center gap-2 ">
                        ...
                        <button wire:click="openAddNewGroup('single')" type="button" class="px-5 py-2.5 text-sm font-medium w-full text-white inline-flex items-center justify-center bg-amber-600 hover:bg-amber-700 focus:ring-4 focus:outline-none focus:ring-amber-500 rounded-lg text-center @if($newGroupType == 'single') bg-amber-700 @endif">
                            <svg class="w-3.5 h-3.5 text-white mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    ...
                    @if($addNewGroup)
                        <div wire:transition class="bg-white">
                            ...
                            @if($newGroupType == 'group')
                                <div class="mt-2">
                                    <input type="text" name="" id="group_name" placeholder="Group name" wire:model="group_new" class="border block w-full px-4 py-3 placeholder-gray-500 border-gray-300 rounded-lg outline-px focus:outline-amber-500" />
                                    @error('group_new')
                            ...
                            <button wire:click="createGroup" type="button" class="mt-4 px-2 py-1.5 w-full text-sm font-sm text-white inline-flex items-center justify-center bg-amber-600 hover:bg-amber-700 focus:ring-4 focus:outline-none focus:ring-amber-500 rounded text-center ">
                                <svg class="w-3.5 h-3.5 text-white mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                ...
                <div>    
                    @foreach ($individual_chats as $chats)
                        @php
                            $sender = $chats->user1_id == auth()->user()->id ? $chats->getUser2 : $chats->getUser1;
                        @endphp
                        <div class="mt-6 space-y-5">
                            <div wire:click="openChat('single', {{$chats->id}})" class="relative overflow-hidden transition-all duration-200 bg-white border border-gray-200 rounded-lg hover:shadow-lg hover:bg-gray-50 hover:-translate-y-1">
                                <div class="p-2 sm:p-4">
                                    <div class="flex items-start">
                                        @if($sender->avatar)
                                            <img class="object-cover w-20 h-20 rounded-lg shrink-0" src="{{ asset('storage/'.$sender->avatar) }}" alt="" />
                                        @else
                                            <div class="inline-flex items-center w-20 h-20 justify-center rounded-lg bg-gray-200 px-10">
                                                <span class="text-3xl font-medium text-gray-500 leading-none uppercase">{{ $sender->username[0] }}</span>
                                            </div>
                                        @endif
                                        <div class="ml-5 w-full flex flex-col gap-2">
                                            <div class="flex items-start sm:items-center justify-between gap-0 sm:gap-4 w-full flex-col sm:flex-row">
                                                ...
                                                <p class="text-sm font-medium text-gray-900 flex items-center gap-3">{{timePassed($chats->getLastChat->created_at ?? null)}}</p>
                                            </div>

                                            <p class="text-sm font-medium text-gray-500 line-clamp-2">
                                                {{$chats->getLastChat->message ?? 'No messages yet with '.$sender->username. ' .Click to start a conversation'}}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @foreach ($group_chats as $group_member)
                        <div class="mt-6 space-y-5">
                            <div wire:click="openChat('group', {{$group_member->group_id}})" class="relative overflow-hidden transition-all duration-200 bg-white border border-gray-200 rounded-lg hover:shadow-lg hover:bg-gray-50 hover:-translate-y-1">
                                <div class="p-4">
                                    <div class="flex items-start">
                                        <div class="grid grid-cols-2 gap-2 w-24 h-20">
                                            @foreach ($group_member->getGroup->getMembers->take(3) as $group_member)
                                                @php
                                                    $member = $group_member->getUser;
                                                @endphp
                                                @if($member->avatar)
                                        ...
                                        <div class="ml-5 w-full flex flex-col gap-2">
                                            <div class="flex items-start sm:items-center justify-between gap-0 sm:gap-4 w-full flex-col sm:flex-row">
                                                ...
                                                <p class="text-sm font-medium text-gray-900 flex items-center gap-3">{{timePassed($group_member->getGroup->getLastMessage->created_at ?? null)}}</p>
                                            </div>

                                            <p class="text-sm font-medium text-gray-500 line-clamp-2">
                                                {{$group_member->getGroup->getLastMessage->message ?? 'No messages yet in '.$group_member->getGroup->name. ' .Click to start a conversation'}}
                                            </p>
                                        </div>
                                    </div>
                                </div>
